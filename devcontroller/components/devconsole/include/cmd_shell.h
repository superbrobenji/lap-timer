 ...
 * prints `ERR shell has no json form` and returns 1 without touching the link. Otherwise, if a
 * flashctl push is in flight (FLASHCTL_PUSHING), prints "busy (push in flight)" and returns 1
 * without touching the link at all (I4, final review) -- a 10-minute shell session must not be
 * able to stall a web push invisibly behind linkhost's request mutex. Else takes linkhost's
 * bridge (linkhost_bridge_begin/_end); a busy link prints "busy" and returns 1.
